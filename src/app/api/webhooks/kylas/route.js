...
export async function POST(request) {
  try {
    ...
    // 2. Log payload to DB if Test Mode is enabled (Do this BEFORE auth check so we can debug failed requests)
      ...
      await prisma.webhookLog.create({
        data: {
          ...
          payload: JSON.stringify({ 
            _test_meta: { authHeader_received: authHeader },
            ...body 
          })
        }
      });
    ...
    // 3. Enforce Authentication if configured
